...
def Dilate_mask(mask, num_objects):
    ...
    for o in range(num_objects+1):  # include bg scribbles
        ...
        dmask = cv2.dilate(bmask, cv2.getStructuringElement(
...
def load_frames(path, size=None, num_frames=None):
    ...
    for i, fname in enumerate(fnames):
        if size:
            frame_list.append(np.array(Image.open(fname).convert('RGB').resize(
                (size[0], size[1]), Image.BICUBIC), dtype=np.uint8))
        else:
            frame_list.append(
...
def overlay_davis(image, mask, rgb=[255, 0, 0], cscale=2, alpha=0.5):
    ...
    foreground = im_overlay*alpha + \
        np.ones(im_overlay.shape)*(1-alpha) * \
...
def checkerboard(img_size, block_size):
    width = int(np.maximum(
        np.ceil(img_size[0] / block_size), np.ceil(img_size[1] / block_size)))
...
def overlay_color(image, mask, rgb=[255, 0, 255]):
    ...
    board = np.ones(image.shape, dtype=np.uint8) * \
        np.array(rgb, dtype=np.uint8)[None, None, :]
    binary_mask = (mask == 1)
    # Compose image
    board[binary_mask] = im_overlay[binary_mask]
